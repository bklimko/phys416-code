...
phi[int(N/4)-1,int(N/4)-1:int(3*N/4)-1] = 1 # capacitor plates at potential one
phi[int(3*N/4)-1,int(N/4)-1:int(3*N/4)-1] = 1

mask = np.ones((N,N))
# set areas that do not get recalculated (hold outer box and capacitor plates at respective potentials)
mask[:,0] = 0
mask[:,-1] = 0
mask[0, :] = 0
mask[-1, :] = 0
mask[int(N/4)-1,int(N/4)-1:int(3*N/4)-1] = 0 
mask[int(3*N/4)-1,int(N/4)-1:int(3*N/4)-1] = 0
